...
pub fn main(init: std.process.Init) !void {
    ...
    const read_len = try parsed.inp.readPositionalAll(init.io, input_bytes, 0);
    if (input_bytes.len != read_len) {
        std.process.fatal("expected size {}, found size {}", .{ input_bytes.len, read_len });
    }
