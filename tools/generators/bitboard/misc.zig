...
pub const b_magic = std.EnumArray(base.types.Square, base.types.Square.Set.Tag).init(.{
...
pub const b_offset = std.EnumArray(base.types.Square, u32).init(.{
...
pub const r_magic = std.EnumArray(base.types.Square, base.types.Square.Set.Tag).init(.{
...
pub const r_offset = std.EnumArray(base.types.Square, u32).init(.{
...
fn genAtk(comptime pt: base.types.Ptype,
  s: base.types.Square,
...
fn genIdx(comptime pt: base.types.Ptype,
  ...
  b: base.types.Square.Set) base.types.Square.Set.Tag {
	return switch (pt) {
		...
		.bishop, .rook => blk: {
			const is_r = pt == .rook;
			const magic = if (is_r) r_magic.getPtrConst(s).* else b_magic.getPtrConst(s).*;
			const nmask = if (is_r) r_nmask.getPtrConst(s).* else b_nmask.getPtrConst(s).*;
