...
pub const mg_tbl = mg_wrapper: {
	...
	break :mg_wrapper Array(comptime_int).init(.{
		.nil = std.EnumArray(misc.types.Square, comptime_int).initFill(0),
		...
		.all = std.EnumArray(misc.types.Square, comptime_int).initFill(0),
	});
};

pub const eg_tbl = eg_wrapper: {
	@setEvalBranchQuota(1 << 12);
	break :eg_wrapper Array(comptime_int).init(.{
		.nil = std.EnumArray(misc.types.Square, comptime_int).initFill(0),
...
pub const tbl = init: {
	...
	var tmp = std.mem.zeroInit(Array(engine.evaluation.Pair), .{});
...
fn Array(comptime T: type) type {
	return std.EnumArray(misc.types.Ptype, std.EnumArray(misc.types.Square, T));
}
