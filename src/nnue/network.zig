...
pub const Default = extern struct {
    ...
    const precision: comptime_int = blk: {
        const l1o_max: comptime_float = q.v * q.v;
        const l2w_max: comptime_float = @round(q.v * 1.98);
        const acc_max: comptime_float = l1o_max * l2w_max * l2s * 2;
        const i32_max: comptime_float = std.math.maxInt(i32);
        break :blk @floor(@log2(i32_max / acc_max));
    };
    ...
    fn activateL1(
        self: *const Self,
        stm_inputs: *align(512) const [l1s]i16,
        ntm_inputs: *align(512) const [l1s]i16,
        l1: []align(page_size) u8,
    ) void {
        _ = self;
        const inputs: [types.Color.num]*align(512) const [l1s]i16 = .{ stm_inputs, ntm_inputs };
        ...
        for (inputs, 0..) |input, which| {
            ...
            while (i < half) : (i += simd.Vec(i8).len) {
                ...
                const d: simd.Vec(u8).Slice = @alignCast(l1[offset + i ..]);
                packus.store(d);
            }
        }
    }
        ...
        while (i + unroll * 2 <= nnz.slice().len) : (i += unroll * 2) {
            for (acc[0..], 0..) |*rolled_lanes, j| {
                ...
                for (rolled_lanes[0..], 0..) |*lane, k| {
                    ...
                    const ws: [2]simd.Vec(i8).ConstSlice = .{
                        @alignCast(wgts[indices[0] * acc_lanes * stride + offset ..]),
                        @alignCast(wgts[indices[1] * acc_lanes * stride + offset ..]),
                    };
                    lane.* = simd.dpbusd(lane.*, vs[0], .load(ws[0]));
                    lane.* = simd.dpbusd(lane.*, vs[1], .load(ws[1]));
                }
            }
            ...
            for (acc[0..], 0..) |*lane, j| {
                ...
                const w: simd.Vec(i8).ConstSlice =
                    @alignCast(wgts[idx * acc_lanes * stride + offset ..]);
                lane[0] = simd.dpbusd(lane[0], v, .load(w));
            }
        }

        const out_vecs: *align(page_size) [acc.len * 2]simd.Vec(i32) = @ptrCast(l2);
        for (
            acc[0..],
            out_vecs[acc.len * 0 ..][0..acc.len],
            out_vecs[acc.len * 1 ..][0..acc.len],
            0..,
        ) |*lane, *first, *second, k| {
            ...
            second.* = clamped.mul(clamped).shr(shift * 2 - precision);
        }
    }
    ...
    fn forwardL2(
        self: *const Self,
        ob: usize,
        l2: *align(page_size) const [l2s * 2]i32,
        ...
        for (l2, wgts) |scalar, *row| {
            for (acc, 0..) |*lane, i| {
                const w: simd.Vec(i32).ConstSlice = @alignCast(row[i * simd.Vec(i32).len ..]);
                const v: simd.Vec(i32) = .splat(scalar);
                lane.* = v.mul(.load(w)).add(lane.*);
            }
        } else for (acc, 0..) |*lane, i| {
            const b: simd.Vec(i32).ConstSlice = @alignCast(self.l2b[ob][i * simd.Vec(i32).len ..]);
            lane.* = lane
                .shr(precision)
                .add(.load(b))
                .crelu(q.pow(3));
    ...
    fn forwardL3(
        ...
        for (lanes, 0..) |v, i| {
            const w: simd.Vec(i32).ConstSlice = @alignCast(self.l3w[ob][i * simd.Vec(i32).len ..]);
            acc = v.mul(.load(w)).add(acc);
        }
