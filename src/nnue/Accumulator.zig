...
pub fn update(
    ...
    while (i < network.Default.l1s) : (i += native_len) {
        const vec: *Native = @ptrCast(@alignCast(a[i..][0..native_len]));
        var acc: Native = @splat(0);
        defer vec.* +%= acc;

        for (adds.constSlice()) |add_i| {
            const v: *const Native = @ptrCast(@alignCast(wgts[add_i][i..][0..native_len]));
            acc +%= v.*;
        }

        for (subs.constSlice()) |sub_i| {
            const v: *const Native = @ptrCast(@alignCast(wgts[sub_i][i..][0..native_len]));
            acc -%= v.*;
        }
    }
}
