...
var atk: [88772]types.Square.Set align(std.atomic.cache_line) = undefined;
...
fn bGenIdx(s: types.Square, b: types.Square.Set) types.Square.Set.Int {
    ...
    const mask = b_tbl.getPtrConst(s).mask;
    return b.bwa(mask).int() *% magic >> (64 - 9);
}

fn rGenIdx(s: types.Square, b: types.Square.Set) types.Square.Set.Int {
    const magic = r_tbl.getPtrConst(s).magic;
    const mask = r_tbl.getPtrConst(s).mask;
    return b.bwa(mask).int() *% magic >> (64 - 12);
}
