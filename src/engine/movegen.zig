...
pub const Picker = struct {
    ...
    fn pick(self: *Picker) ?Move.Scored {
        ...
        while (i + native_len < scores.len) : ({
            i += native_len;
            indices +%= @splat(native_len);
        }) {
            const rhs = evaluation.score.withIndices(scores[i..][0..native_len].*, indices);
            bests = @max(bests, rhs);
        }

        var best = @reduce(.Max, bests);
        while (i < scores.len) : (i += 1) {
            const rhs = evaluation.score.withIndex(scores[i], i);
            best = @max(best, rhs);
        }
...
pub const RootMove = struct {
    ...
    pub const List = struct {
        ...
        pub fn find(
            self: anytype,
            m: Move,
        ) ?types.SameMutPtr(@TypeOf(self), *List, *RootMove) {
