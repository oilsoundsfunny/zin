    ...
    move_loop: while (mp.next()) |sm| {
            ...
            tte.flag != .upperbound)
        {
            ...
            if (se_score < sb) extend: {
                const margins: [3]evaluation.score.Int = if (is_noisy) .{
                    ...
                    params.values.qext_noisy + params.values.qext_pv * @intFromBool(is_pv),
                } else .{
                    ...
                    params.values.qext_quiet + params.values.qext_pv * @intFromBool(is_pv),
                };

                e += 1;
                e += if (se_score < sb - margins[0]) 1 else break :extend;
                e += if (se_score < sb - margins[1]) 1 else break :extend;
                e += if (se_score < sb - margins[2]) 1 else break :extend;
            } else if (sb >= b) {
                const min = evaluation.score.loss + 1;
                const max = evaluation.score.win - 1;
